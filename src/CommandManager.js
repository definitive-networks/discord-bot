...
class CommandManager {
  ...
  create(data) {
    if (Command.isValid(this.client, data)) {
      this.cache.set(data.name, new Command(this.client, data));
    } else {
      this.client.emit('warn', `Command invalid, unable to create:\n${data}`);
    }
  }
  ...
  shouldHandle(event) {
    if (event.commandId) {
      return this.cache.has(event.commandName);
    } else {
      switch (true) {
        case event.partial:
        case event.author.bot:
        case event.author.id === this.client.user.id: {
          return false;
        }
        default: {
          return true;
  ...
  shouldDeleteSlash(slash) {
    ...
    const commandId = this.client.database.commands.get(`${slash.guild ? slash.guildId : 'global'}.${command.name}`);
  ...
  async deleteSlash(slash) {
    ...
    try {
      ...
      if (this.client.database.commands.has(`${guild ? slash.guildId : 'global'}.${slash.name}`)) {
        this.client.database.commands.delete(`${guild ? slash.guildId : 'global'}.${slash.name}`);
        this.client.emit(
          'debug',
